...
**Транспорты и обновление Xray:**
- 🐛 Поддержка `xhttp`: раньше для него не создавалась секция настроек, и
  `path`/`host` из ссылки терялись. Добавлены также `httpupgrade` и
  `splithttp` (прежнее имя `xhttp`), плюс параметр `mode` из VLESS-ссылки
- 🐛 `h2`/`http` удалены из Xray («migrated to XHTTP stream-one»), и профили
  с ними переставали работать. Теперь переводятся на `xhttp` с
  `mode: stream-one` автоматически
- 🐛 Установщик обновляет Xray, а не только доустанавливает: раньше
  проверялось лишь наличие файла, поэтому однажды поставленный бинарник не
  менялся никогда. У пользователя так остался 1.8.7 (2023 год), не знающий
  `xhttp`, — подключение падало с «unknown transport protocol»
- ✅ `check-installation.bat` показывает версию Xray и предупреждает, если
  она отстала от актуальной

**Linux (в том числе Steam Deck):**
- 🐛 Регистрация в Flatpak-браузерах. Манифест клался только в
  `~/.config/<браузер>/`, а Flatpak-браузеры читают его из
  `~/.var/app/<id>/config/...` — установка проходила успешно, но расширение
  сообщало «native host не установлен». Добавлены Chrome, Chromium, Brave,
  Edge, Vivaldi и Яндекс; каталог создаётся только для реально установленных
